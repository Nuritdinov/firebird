 ...
 **************************************/
	...
	if ((RIDS) relation->rel_id == rel_priv)
	{
		...
		EVL_field(0, rpb->rpb_record, f_prv_rname, &desc);
		MOV_get_metaname(tdbb, &desc, object_name.object);	// FIXME:
 ...
 **************************************/
	...
	static std::map<USHORT, std::vector<USHORT>> schemaFields = {
		{rel_database, {f_dat_charset_schema}},
		{rel_fields, {f_fld_schema}},
		{rel_segments, {f_seg_schema}},
		{rel_indices, {f_idx_schema, f_idx_foreign_schema}},
		{rel_rfr, {f_rfr_schema, f_rfr_field_source_schema}},
		{rel_relations, {f_rel_schema}},
		{rel_vrel, {f_vrl_schema, f_vrl_rname_schema}},
		{rel_classes, {f_cls_schema}},
		{rel_triggers, {f_trg_schema}},
		{rel_dpds, {f_dpd_schema, f_dpd_o_schema}},
		{rel_funs, {f_fun_schema}},
		{rel_args, {f_arg_schema, f_arg_rel_schema, f_arg_field_source_schema}},
		{rel_msgs, {f_msg_schema}},
		{rel_priv, {f_prv_rel_schema, f_prv_user_schema}},
		{rel_gens, {f_gen_schema}},
		{rel_dims, {f_dims_schema}},
		{rel_rcon, {f_rcon_schema}},
		{rel_refc, {f_refc_schema}},
		{rel_ccon, {f_ccon_schema}},
		{rel_procedures, {f_prc_schema}},
		{rel_prc_prms, {f_prm_schema, f_prm_rel_schema, f_prm_field_source_schema}},
		{rel_charsets, {f_cs_schema, f_cs_def_coll_schema}},
		{rel_collations, {f_coll_schema, f_coll_base_collation_schema}},
		{rel_exceptions, {f_xcp_schema}},
		{rel_packages, {f_pkg_schema}},
		{rel_pub_tables, {f_pubtab_schema}},
	};
	...
	{
		for (const auto fieldId : relSchemaFields->second)
		{
			if (rpb->rpb_record->isNull(fieldId) && !EVL_field(0, rpb->rpb_record, fieldId, &schemaDesc))
			{
				if (desc.isNull())
					desc.makeText(strlen(SYSTEM_SCHEMA), CS_METADATA, (UCHAR*) SYSTEM_SCHEMA);
	...
	if (needDfw(tdbb, transaction))
	{
		...
		case rel_priv:
			...
			// FIXME: DFW_post_work(transaction, dfw_grant, &desc, object_id);
			break;
