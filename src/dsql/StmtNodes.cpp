...
DeclareSubFuncNode* DeclareSubFuncNode::dsqlPass(DsqlCompilerScratch* dsqlScratch)
{
	...
	for (NestConst<ParameterClause>* i = dsqlBlock->parameters.begin(); i != dsqlBlock->parameters.end(); ++i)
	{
		...
		sigParam.name = param->name.object;
		...
		if (!implemetingForward)
		{
			...
			dsqlFunction->udf_arguments.add().name = param->name.object;
		}
	...
	Array<NestConst<ParameterClause> >& paramArray)
{
	...
	for (NestConst<ParameterClause>* i = paramArray.begin(); i != paramArray.end(); ++i)
	{
		...
		dsqlScratch->appendNullString(param->name.object.c_str());
...
ExecProcedureNode* ExecProcedureNode::dsqlPass(DsqlCompilerScratch* dsqlScratch)
{
	...
	{
		DeclareSubProcNode* subProcedure = dsqlScratch->getSubProcedure(dsqlName.object);
...
void ExecBlockNode::genBlr(DsqlCompilerScratch* dsqlScratch)
{
	...
	if (!subRoutine)
	{
		...
		for (FB_SIZE_T i = 0; i < parameters.getCount(); ++i)
		{
			...
			dsqlScratch->makeVariable(parameter->type, parameter->name.object.c_str(),
	...
	switch (exception->type)
	{
		case ExceptionItem::GDS_CODE:
			if (xcpCode == isc_check_constraint)
			{
				...
				ERR_post(Arg::Gds(xcpCode) << constraintName << relationName.toString());
			}
			else
	...
	const Array<NestConst<ParameterClause>>* outputParameters)
{
	...
	for (const auto parameters : {inputParameters, outputParameters})
	{
		if (parameters)
		{
			...
			{
				if (parameter->name.object.hasData())	// legacy UDFs has unnamed parameters
				{
					FB_SIZE_T pos;
					if (!names.find(parameter->name.object.c_str(), pos))
						names.insert(pos, parameter->name.object.c_str());
					else
					{
						ERRD_post(
							Arg::Gds(isc_sqlerr) << Arg::Num(-637) <<
							Arg::Gds(isc_dsql_duplicate_spec) << parameter->name.object);
					}
				}
			}
		}
	...
	if (node)
	{
		for (const auto statement : node->statements)
		{
			...
			if (auto varNode = nodeAs<DeclareVariableNode>(statement))
				name = varNode->dsqlDef->name.object.c_str();
			...
			if (notMatched.fields.hasData())
			{
				...
				for (const auto& field : fields)
				{
						...
						tmpCtx->ctx_context != storeContext->ctx_context))
					{
						...
						PASS1_field_unknown((badRel ? badRel->rel_name.object.c_str() : NULL),	// FIXME:
		...
		for (const auto item : node->rse->dsqlStreams->items)
		{
			...
			if (auto relNode = nodeAs<RelationSourceNode>(item))
			{
				...
				if (const auto* const relation = context->ctx_relation)
				{
					...
					parameter->par_dbkey_relname = relation->rel_name.object;	// FIXME:
					...
					parameter->par_rec_version_relname = relation->rel_name.object;	// FIXME:
					parameter->par_context = context;
...
static dsql_par* dsqlFindDbKey(const DsqlDmlStatement* statement, const RelationSourceNode* relation_name)
{
	...
	for (FB_SIZE_T i = 0; i < message->msg_parameters.getCount(); ++i)
	{
		...
		if (parameter->par_dbkey_relname.hasData() && parameter->par_dbkey_relname == relName.object)	// FIXME:
		{
			if (candidate)
				return NULL;
...
static dsql_par* dsqlFindRecordVersion(const DsqlDmlStatement* statement, const RelationSourceNode* relation_name)
{
	...
	for (FB_SIZE_T i = 0; i < message->msg_parameters.getCount(); ++i)
	{
		...
		if (parameter->par_rec_version_relname.hasData() &&
			parameter->par_rec_version_relname == relName.object)	// FIXME:
		{
			if (candidate)
				return NULL;
	...
	const RelationSourceNode* relation_name)
{
	...
	for (auto& recSource : temp->items)
	{
		if (auto relNode = nodeAs<RelationSourceNode>(recSource))
		{
			...
			if (relation->rel_name == relName)	// FIXME:
			{
				if (context)
				{
