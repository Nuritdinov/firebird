...
void DsqlCompilerScratch::putLocalVariableDecl(dsql_var* variable, DeclareVariableNode* hostParam,
	const QualifiedName& collationName)
{
	...
	Array<NestConst<ParameterClause> >& returns)
{
	if (parameters.hasData())
	{
		...
		for (FB_SIZE_T i = 0; i < parameters.getCount(); ++i)
		{
			...
			putDebugArgument(fb_dbg_arg_input, i, parameter->name.object.c_str());	// TODO: ?
			...
			makeVariable(parameter->type, parameter->name.object.c_str(),	// TODO: ?
	...
	if (returns.hasData())
	{
		for (FB_SIZE_T i = 0; i < returns.getCount(); ++i)
		{
			...
			putDebugArgument(fb_dbg_arg_output, i, parameter->name.object.c_str());	// TODO: ?
			putType(parameter->type, true);
...
SelectExprNode* DsqlCompilerScratch::findCTE(const MetaName& name)	// FIXME: ?
{
...
bool DsqlCompilerScratch::pass1RelProcIsRecursive(RecordSourceNode* input)
{
	...
	const bool recursive = currCte->alias == relName.object.c_str();	// FIXME: ?
