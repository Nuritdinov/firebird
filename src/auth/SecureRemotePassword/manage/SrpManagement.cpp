...
private:
	...
	{
		const char* script[] = {
			"CREATE TABLE SYSTEM.PLG$SRP (PLG$USER_NAME SYSTEM.SEC$USER_NAME NOT NULL PRIMARY KEY, "
			...
			"PLG$ACTIVE BOOLEAN)"
			,
			"CREATE VIEW SYSTEM.PLG$SRP_VIEW AS "
			"SELECT PLG$USER_NAME, PLG$VERIFIER, PLG$SALT, PLG$COMMENT, "
			"   PLG$FIRST, PLG$MIDDLE, PLG$LAST, PLG$ATTRIBUTES, PLG$ACTIVE "
			"FROM SYSTEM.PLG$SRP WHERE RDB$SYSTEM_PRIVILEGE(USER_MANAGEMENT) "
			"   OR CURRENT_USER = PLG$SRP.PLG$USER_NAME"
			,
			"GRANT ALL ON SYSTEM.PLG$SRP TO VIEW SYSTEM.PLG$SRP_VIEW"
			,
			"GRANT SELECT ON SYSTEM.PLG$SRP_VIEW TO PUBLIC"
			,
			"GRANT UPDATE(PLG$VERIFIER, PLG$SALT, PLG$FIRST, PLG$MIDDLE, PLG$LAST, "
			"   PLG$COMMENT, PLG$ATTRIBUTES) ON SYSTEM.PLG$SRP_VIEW TO PUBLIC"
			,
			"GRANT ALL ON SYSTEM.PLG$SRP_VIEW TO SYSTEM PRIVILEGE USER_MANAGEMENT"
			,
			NULL
		};
	...
	int execute(Firebird::CheckStatusWrapper* status, Firebird::IUser* user, Firebird::IListUsers* callback)
	{
		try
		{
			...
			case Firebird::IUser::OP_USER_ADD:
				{
					const char* insert =
						"INSERT INTO system.plg$srp_view(PLG$USER_NAME, PLG$VERIFIER, PLG$SALT, PLG$FIRST, PLG$MIDDLE, PLG$LAST,"
			...
			case Firebird::IUser::OP_USER_MODIFY:
				{
					Firebird::string update = "UPDATE system.plg$srp_view SET ";
			...
			case Firebird::IUser::OP_USER_DELETE:
				{
					const char* del = "DELETE FROM system.plg$srp_view WHERE PLG$USER_NAME=?";
					Firebird::IStatement* stmt = NULL;
					try
					{
